# -*- coding: utf-8 -*-
"""
build_scene_assets.py - TASK 6B 通过版本

【场景素材自动生成模块 + asset_selector 集成】

目的：
当 data/current/scene_assets.json 缺失或无效时，
从 scene_plan.json 自动生成最小可用版本，
并调用 asset_selector 自动匹配素材

TASK 6B 集成特性：
- 自动调用 asset_selector.select_asset_for_text()
- 优先使用匹配到的图像资源，否则 fallback 到 text_card
...
def build_minimal_scene_assets(scenes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for idx, scene in enumerate(scenes):
        ...
        asset_record = {
            # 【video_engine 必需字段】
            "type": asset_type,           # ← video_engine 读取 scene.get("type")
            "file": asset_file,           # ← video_engine 读取 scene.get("file")
            ...
            "matched_keyword": match_keyword  # 记录匹配到的关键词（调试用）
        }
...
def build_scene_assets() -> None:
    ...
    try:
        ...
        print(f"   - 素材文件：由 asset_selector 或 fallback 决定")
